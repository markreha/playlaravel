...
class Login6Controller extends Controller
{
     public function index(Request $request)
    {
        ...
        try
        {
           ...
           // Validate the Form Data (note will automatically redirect back to Login View if errors)
            ...
            if($status)
            {
                MyLogger2::info("Exit Login6Controller.index() with login passed");
                $Data = ['model' => $user];
                return view('loginPassed6')->with($Data);
            }
            else
            {
                MyLogger2::info("Exit Login6Controller.index() with login failed");
                return view('loginFailed6');
            }
        }
